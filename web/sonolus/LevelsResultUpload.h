auto OneDriveLevelsResultUpload = [](client_conn conn, http_request request, param argv){
    if (!checkLogin(request)) quickSendMsg2(401, "Unauthorized.");
    ...
    for (int i = 1; i < fileParts.size() - 1; i++) {
        ...
        if (hash != realHash) quickSendMsg2(403, "Hash mismatch.");
        onedrive_uploader(file);
        files[hash] = file;
    }

    string key = request.argv["sonolus-upload-key"];
    vector<Record<LevelItem> > records = recordsList<LevelItem>("name = \"" + key + "\"", "");
    if (records.size() == 0) quickSendMsg2(404, "Record not found.");
